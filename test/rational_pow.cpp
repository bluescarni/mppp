...
struct pow_tester {
    ...
    inline void operator()(const S &) const
    {
        ...
        REQUIRE(std::abs(mppp::pow(rational{2}, std::complex<long double>{2}) - std::complex<long double>{4, 0})
                <= std::numeric_limits<long double>::epsilon() * 1000);
        REQUIRE(std::abs(mppp::pow(std::complex<long double>{2}, rational{2}) - std::complex<long double>{4, 0})
                <= std::numeric_limits<long double>::epsilon() * 1000);
#endif
    }
};
