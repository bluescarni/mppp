...
if [[ "${MPPP_BUILD}" == "ReleaseGCC48" ]]; then
    ...
    cmake -DCMAKE_PREFIX_PATH=$deps_dir -DCMAKE_BUILD_TYPE=Debug -DMPPP_BUILD_TESTS=yes -DMPPP_WITH_MPFR=yes -DMPPP_WITH_ARB=yes -DMPPP_WITH_MPC=yes -DMPPP_WITH_QUADMATH=yes -DMPPP_ENABLE_IPO=yes ../ || cat /home/travis/build/bluescarni/mppp/build/CMakeFiles/CMakeError.log
    make -j2 VERBOSE=1;
    ctest -V;
