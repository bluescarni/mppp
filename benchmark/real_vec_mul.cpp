...
static inline std::tuple<std::vector<T>, std::vector<T>, std::vector<T>> get_init_vectors(double &init_time)
{
    ...
    std::fill(v1.begin(), v1.end(), T("1.1", 256));
    std::fill(v2.begin(), v2.end(), T("1.1", 256));
    std::cout << "\nInit runtime: ";
...
int main()
{
                    ...
                    "    data = [";
    {
        ...
        simple_timer st1;
        double init_time;
        auto p = get_init_vectors<real>(init_time);
...
#if defined(MPPP_BENCHMARK_BOOST)
    {
        ...
        s += "['Boost (mpfr_float)','init'," + std::to_string(init_time) + "],";
        {
            simple_timer st2;
            mpfr_float ret;
            ::mpfr_set_prec(ret.backend().data(), 256);
            ::mpfr_set_si(ret.backend().data(), 0, MPFR_RNDN);
            for (auto i = 0ul; i < size; ++i) {
